...
def is_object_active(scene, obj):
    return sum([scene.layers[i] * obj.layers[i] for i in range(20)])>0

def getMeshModName(obj):
    if len(obj.modifiers)==1 and obj.modifiers[0].type=='ARMATURE':
        name = obj.data.name
    if obj.modifiers:
        name = obj.data.name + '_moded_by_' + obj.name
    else:
        name = obj.data.name
    dic = {}
    dic['а'] = 'a'
    ...
    for i in dic.keys():
        cap_key = i.capitalize()
        cap_val = dic[i].capitalize()
        name = name.replace(i, dic[i])
        name = name.replace(cap_key, cap_val)
    return name
...
def save_mesh(ob, scene, custom_name=None):
    ...
    for face in pols:
        ...
        for vert in li:
            ...
            t_ozhidanie = vert.tangent.cross(vert.normal)
            t_realnost  = vert.bitangent
            diff = t_realnost.dot(t_ozhidanie) < 0.99
...
def texturesArray():
    ...
    for i in bpy.data.textures:
        ...
        dds_fname = fname[:fname.rfind('.')] + '.dds'
        prog = "nvcompress" if platform.system().lower()=='windows' else './nvcompress'
        if not os.path.isfile(dds_fname):
            if i.name.lower().find('norm')>-1:
                call(prog + ' -fast -bc1n "{}"'.format(fname), shell=True)
            else:
                call(prog + ' -fast -bc1 "{}"'.format(fname), shell=True)
    return tex_count,textures

def materialsArray():
    materials = pack('<I', len(bpy.data.materials) + 1)
    materials+= b'default\n'  # Имя материала
    materials+= b'\n'         # Имя текстуры
    materials+= b'\n'         # Имя текстуры свечения
    materials+= b'\n'         # Имя текстуры шероховатости
    materials+= b'\n'         # Имя текстуры зеркальности
    materials+= b'\n'         # Имя карты рельефа
    materials+= b'\n'         # Имя текстуры освещения

    materials+= pack('<3f', 0.5,0.5,0.5)    # Diffuse reflection
    materials+= pack('<3f', 0.0,0.0,0.0)    # Specular reflection
    materials+= pack('<f',  0.0)      # Шероховатость
    materials+= pack('<f',  0.0)      # Металлик
    materials+= pack('<f',  0.0)      # Френнель
    ...
    for mat in bpy.data.materials:
        ...
        for tex in mat.texture_slots:
            if tex is not None:
                ...
                if tex.use_map_emit:
                    gtex = tex.name.encode()
                if tex.use_map_ambient:
                    lmtex = tex.name.encode()
                if tex.use_map_hardness:
                    rtex = tex.name.encode()
                if tex.use_map_raymir or tex.use_map_mirror:
                    mtex = tex.name.encode()
        ...
        material += pack("<f",   1.0-mat.specular_intensity)
        material += pack("<f",   mat.raytrace_mirror.reflect_factor)
        material += pack("<f",   mat.raytrace_mirror.fresnel)
        material += pack("<f",   mat.emit)
        material += pack("<f",   (1-mat.alpha) * mat.use_transparency)
        material += pack("<d",   mat.physics.friction)
        
        materials += material
    return len(bpy.data.materials) + 1, materials
...
def lightsArray(scene):
    ...
    for i in scene.objects:
        if (i.type == 'LAMP' or i.type == 'LIGHT') and types[i.data.type] is not None:
            ...
            light += pack('?', not is_object_active(i, scene))
            light += parent_name.encode() + b'\n'
            light += i.parent_bone.encode() + b'\n'
            ...
            if hasattr(l,'spot_size'):
                light += pack('<f', l.spot_size/2 * (1.0-l.spot_blend))
                light += pack('<f', l.spot_size/2)
            else:
                light += pack('<f', 0.0)
                light += pack('<f', 0.0)
            ...
            lights += light
            lights_count += 1
    return lights_count, pack('<I', lights_count) + lights
...
def objectsArray(scene, save_meshes):
    ...
    for i in scene.objects:
        if i.type == 'MESH':
            ...
            obj += pack('?', not is_object_active(i, scene))
            obj += pack('?', i.hide_render)
            obj += pack('?', i.game.use_ghost)
            obj += parent_name.encode() + b'\n'
            obj += i.parent_bone.encode() + b'\n'
    ...
    return obj_count, pack("<I", obj_count) + objects
...
def skeletonsArray(scene):
    ...
    for i in scene.objects:
        ...
        skeleton += pack('?', not is_object_active(i, scene))
    ...
    return skel_count, pack('<I', skel_count) + skeletons
...
def camera(scene):
    ...
    pbname = cam.parent_bone.encode() + b'\n'
    ...
    return cname + pname + pbname + cmat + cangle
...
def save_action(scene, skel, act_name):
    ...
    try:
        file = open(pjoin(ACTION_PATH, skel.name + '_' + act_name + '.anim'), 'rb')
        h = file.read(8) 
        file.close()
        if h==hash:
        ...
        pass
    file = open(pjoin(ACTION_PATH, skel.name + '_' + act_name + '.anim'), 'wb')
    ...
    for skel in scene.objects:
        if skel.type == 'ARMATURE':
            ...
            for act in bpy.data.actions:
                ...
                if (actBonesNames&bonesNames)==actBonesNames and actBonesNames:
                    actions += (skel.name + "_" + act.name).encode() + b'\n'
                    save_action(scene, skel, act.name)
                    count += 1
                
    return actions, pack('<I', count) + actions
...
def export():
    ...
    print('=== Materials ===')
    mc, mx = materialsArray()
    file.write(mx)
    print('=================\n')

    #print('=== Meshes ======')
    mec, mex = meshesArray(scene)
    file.write(mex)

    print('=== Lights ======')
    lc, lx = lightsArray(scene)
    file.write(lx)
    print('=================\n')

    print('=== Objects =====')
    oc, ox = objectsArray(scene, 1)
    file.write(ox)
    print('=================\n')
    file.close()
    ...
    print('=== Actions =====')
    ta = time()
    ac, ax = actionsArray(scene)
    file.write(ax)
    print(time() - ta,'s.')
    print('=================\n')

    print('=== Skeletons ===')
    sc, sx = skeletonsArray(scene)
    file.write(sx)
    print('=================\n')
    
    print(time() - t1, 'seconds')
    
    '''if tc: print('size of texture', len(tx)/tc)
    if mc: print('size of material', len(mx)/mc)
    if mec: print('size of mesh', len(mex)/mec)
    if ac: print('size of action', len(ax)/ac)
    if lc: print('size of lights', len(lx)/lc)
    if sc: print('size of skel', len(sx)/sc)
    if oc: print('size of object', len(ox)/oc)'''
...
def run():
    ...
    if platform.system()=='Windows':
        call('python37\\python ui_tester.py -scene {}'.format(scene.name), shell=True)
    else:
        call('python3 ui_tester.py -scene {}'.format(scene.name), shell=True)
    #reload(sgm)
