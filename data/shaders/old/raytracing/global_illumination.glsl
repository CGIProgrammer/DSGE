...
vec3 SSGI(vec3 rayHit, vec3 norm)
{
    ...
    {
        vec3 dir = normalize(normalize(blueRand3(tex_map, i)-0.5) + norm);
        if (dot(dir, norm) < 0.0)
        {
            dir *= -1.0;
        }
        ssrt = SSRT2(rayHit,dir);
        result += mix(static_ambient, ssrt.rgb*max(dot(norm,dir),0.0),clamp(ssrt.a, 0.0,1.0));
        samples += ssrt.a;
    }
