...
vec3 getSkyVector(vec2 tm)
{
	vec4 skybox = vec4((tm*2.0-1.0)*5.0,0.0,0.0) * vCameraProjectionInv;
    mat4 sas = inverse(vCameraTransform);
    ...
    //skybox = normalize(skybox);
    return normalize(skybox.xyz);
}

vec2 vectorToUV(vec3 vector)
{
    float horizontal = atan2(vector.x, vector.y) * 2.0 / PI;
    float vertical   = atan2(length(vector.xy), vector.z) * 2.0 / PI;
...
void main() {
    ...
    vec2 uvMouse = vec2(0.5, 0.6);
    uvMouse.x *= AR;
    
   	vec2 uv0 = vectorToUV(getSkyVector(tex_map));
    
    vec2 uv = vectorToUV(getSkyVector(tex_map));
...
/*void main() {
    vec4 skybox = vec4((tex_map*2.0-1.0)*5.0,0.0,0.0) * vCameraProjectionInv;
    mat4 sas = inverse(vCameraTransform);
    skybox.zw = vec2(-5.0, 1.0);
    skybox *= mat4(vec4(sas[0].x,sas[1].x, sas[2].x, 0.0),
                   vec4(sas[0].y,sas[1].y, sas[2].y, 0.0),
                   ...
                   vec4(0.0,0.0,0.0,1.0));
    skybox = normalize(skybox);
    vec2 uv = vec2(2.0 * tex_map.x - 1.0,  -2.0 *  tex_map.y + 1.0);
    vec2 sunpos = vec2(sunx,suny);
    float t = time;
    //uv.y = max(-uv.y, 0.0);
    float horizontal = atan2(skybox.x, skybox.y) / PI;
    float vertical   = atan2(length(skybox.xy), skybox.z) * 2.0 / PI;
    uv.x = horizontal;
    uv.y = vertical;
    fragColor = generate(uv,uv,sunpos,t);
    //fragColor.rgb *= 2.0;
    //fragColor.rgb = fragColor.rgb / (fragColor.rgb + 1.0);
    uv.x = uv.x>1.0 ? 0.0 : uv.x;
    uv.y = uv.y>1.0 ? 0.0 : uv.y;
}*/
