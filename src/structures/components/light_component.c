...
{
    sFrameBufferBind(&light->shadow_buffer, 1);
...
static void sLightShadowCubeRenderCallback(sLightComponentID light, sGameObjectID* draw_list)
{
	...
	laMatrix sides[] = {
		...
		for (int i=0; i<6; i++)
		{
			...
			sFrameBufferBind(&light->shadow_buffer, 2<<i);
			sLightRenderSingleShadowMap(light, draw_list);
		}
}
...
sLightComponentID sLightCreateShadowBuffer(uint16_t size, sLightType lt)
{
    ...
    light->shadow_buffer = sFrameBufferCreate(size, size, 1);
    sTextureID target = 0;
    char name[256];
